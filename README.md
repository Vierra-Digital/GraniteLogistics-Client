...
| `npm run a11y` | accessible names, alt text, heading order, duplicate ids, dangling aria, label association, keyboard reachability, mouse-only controls, dialog semantics | 26 views |
| `npm run shots` | renders 44 screens to `shots/` (gitignored) so they can be looked at | both themes, phone and desktop, empty states, worst-case content, and every step of the sign-in and ordering flows |
